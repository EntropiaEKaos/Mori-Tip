...
const TABS = ["stats", "users", "posts", "inns", "broadcast"] as const;
    ...
    <div className="space-y-4">
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-2xl p-5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-300">Controle total da plataforma Mori</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 flex flex-wrap overflow-hidden">
        {TABS.map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            className={cn(
              "px-4 py-2.5 text-sm font-semibold capitalize",
              tab === t ? "text-cyan-600 border-b-2 border-cyan-500" : "text-slate-500 hover:bg-slate-50",
            )}
          >
            {t === "stats"
              ? "Visão geral"
      ...
      {tab === "stats" && stats && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <StatCard label="Lives criadas" value={stats.lives} icon={<Radio />} color="from-red-500 to-red-700" />
          <StatCard label="Lives ao vivo" value={stats.liveNow} icon={<Radio />} color="from-orange-500 to-red-500" />
          <StatCard label="Pousadas" value={stats.inns} icon={<MapPin />} color="from-amber-500 to-amber-700" />
          <StatCard label="Pendentes" value={stats.pendingInns} icon={<MapPin />} color="from-orange-500 to-orange-700" />
